...
__KNL__ k_tick_t tos_timer_next_expires_get(void)
{
    TOS_CPU_CPSR_ALLOC();
...
__KNL__ void tos_timer_update(void)
{
    k_timer_t *tmr, *tmp;
...
__STATIC__ void timer_task_entry(void *arg)
{
    ...
    while (K_TRUE) {
        next_expires = tos_timer_next_expires_get();
...
__KNL__ k_err_t tos_timer_init(void)
{
#if TOS_CFG_TIMER_AS_PROC > 0u
    return K_ERR_NONE;
#else
